...
```{r}
gost_results_obj_filtered$result <- gost_results_obj_filtered$result %>%
...
```{r}
scores <- all_genes_tested_df$log2FoldChange
names(scores) <- all_genes_tested_df$gene_id
...
```{r}
hallmark_list <- split(
  x = h_gene_sets_filtered$ensembl_gene,
...
```{r}
ggplot(fgseaRes, aes(NES, reorder(pathway, NES))) +
...
```{r}
plotEnrichment(hallmark_list[["HALLMARK_PEROXISOME"]], scores) +
