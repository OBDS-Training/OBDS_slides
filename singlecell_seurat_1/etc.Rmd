...
- `[[` is used to access one column of cell metadata as a `data.frame`.
  Get all the metadata using empty brackets, i.e. `seurat_object[[]]`.

- `$` is used to access one column of cell metadata as a `vector`.

- `FetchData()` is a versatile function that returns per-cell information (e.g., feature expression, dimensionality reduction coordinates, metadata).
...
- Display the entire data.frame of per-cell metadata.
