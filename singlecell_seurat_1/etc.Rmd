...
.pull-right[
```{r, include=TRUE, echo=FALSE, fig.align='center', fig.height=5}
ggplot(FetchData(seurat_object, vars = c("PC_1", "PC_2", "RNA_snn_res.0.5")), aes(PC_1, PC_2, color = RNA_snn_res.0.5)) +
    geom_point(size = 0.2) +
    cowplot::theme_cowplot()
```
]
...
- Despite best efforts, clusters and markers might still reveal residual quality control issues left in the data set.

.pull-left[
```{r, include=TRUE, echo=FALSE, fig.align='center', fig.height=5}
ggplot(FetchData(seurat_object, vars = c("UMAP_1", "UMAP_2", "RNA_snn_res.0.5")), aes(UMAP_1, UMAP_2, color = RNA_snn_res.0.5)) +
    geom_point(size = 0.2) +
    cowplot::theme_cowplot()
```
]
