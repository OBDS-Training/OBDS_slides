...
| `misc` | List of user-defined miscellaneous information. |
...
| Slot | Purpose |
| :--- | :------ |
| `layers` | List of matrix-like objects representing data at various stages of processing (e.g., `counts`, `data`, `scale.data`). |
| `cells` | A `LogMap` object that indicates which cells are present in each layer. |
| `features` | A `LogMap` object that indicates which features are present in each layer. |
| `default` | Index of the default layer. (integer) |
| `assay.orig` | Empty character scalar. Those slots will be used in the future to track the provenance of individual assays. ^[1]^ |
