...
- New accounts on the OBDS server are created with a initial copy of ~/.bashrc file 
  + <span style="color:green;">Show .bashrc template on OBDS server</span>
...
- You will be prompted for: 
  ...
  + A password or passphrase (ideally, always set a password for maximum security)
    - For Git Bash users (Windows users), please leave the passphrase blank (just enter Return key) for now because unlike in macOS, the passphrase cannot be easily stored by an SSH agent so you would need to input it everytime

- The command creates files in **~/.ssh/**:
  + A private key (file), **'id_ecdsa'** 
  + A public key (file), **'id_ecdsa.pub'** 
- Please ensure that the private key has only read and write permissions for user and no permissions for group or other -- file must be secure for the authentication to work
...
- Once the public key has been copied you can log into the host using the SSH key for authentication rather than your password
...
- To avoid typing your SSH key passphrase every time you use the key, you can add your key to an SSH authentication agent
  ...
  + <span style="color:red;">Note that the agent activation and key addition have to be done every time you start a 'session'</span>
    - Say you started a new terminal window, you have to run the two commands again! We will get rid of this later using the SSH config file!
