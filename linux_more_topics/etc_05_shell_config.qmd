...
- New accounts on the cluster are created with a initial copy of ~/.bashrc file (show obds cluster .bashrc)
...
$ ssh -J <jump username>@<jump host> <target username>@<target host>  # Through a jump host 
```
  + -J - If applicable, connecting through a jump (bastion / proxy) host as an intermediary or gateway for extra security
...
::: {.column width="50%"}
  ...
  + 'LocalForward' - specify connection between your machine's port 8787 to the target host's port 8787, which "offers" RStudio
...
```
Host *                            
    IdentityFile ~/.ssh/id_ecdsa  # If using an SSH key pair as a default if not specified later on for a particular host
...
```
  + The pattern * matches all host names, making those parameters global defaults unless overwritten in subsequent declaration
   
- Look at OBDS config file template
