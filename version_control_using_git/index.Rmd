...
# Getting start with <i class="fab fa-git"></i>

Use SSH to open a terminal on the cluster that we use for this training.

The `which` command prints the path to the `git` executable.
What path does it give you?
...
In this step, you will each get (i.e., _clone_) your own copy of the shared repository for this course.
...
$ mkdir -p $HOME/devel
$ cd $HOME/devel
$ git clone git@github.com:OBDS-Training/OBDS_Training_May_2021.git
```
...
- By default, the local directory will be named identically to the repository, it is possible to name it differently using `git clone git@github.com:OBDS-Training/OBDS_Training_May_2021.git local_directory_name`
...
We will not discuss in further details the contents of the `.git` sub-directory.
You can still have a brief look for yourself if you wish.
Make sure that you do not alter anything in that sub-directory.
...
**What do you think will happen when David and Kevin push their respective `master` branch to GitHub?**
...
- You can only create branches with names that are not already used.
...
In the `git` sub-directory (not to be confused with `.git`), create a file called `<username>.md`, replacing `<username>` by your own GitHub user name.

> I included an example `kevinrue.md`, as `kevinrue` is my GitHub user name.

In that file, write you full name, following a hash tag symbol `#` and a space character.

> For me, that looks like `# Kevin Rue-Albrecht`

It is best practice to add a newline character at the end of this line.
<i class="fab fa-git"></i> works best when the last line of each file is empty.
...
```{bash, eval=FALSE, echo=TRUE}
$ git commit -m "Added a file for <username>"
```

```
 1 file changed, 1 insertion(+)
 create mode 100644 git/username.md
```
...
```
origin	git@github.com:OBDS-Training/OBDS_Training_May_2021.git (fetch)
origin	git@github.com:OBDS-Training/OBDS_Training_May_2021.git (push)
```
...
**Do not run this yet, we will do it in a moment.**
...
The first time you push a new branch, you should use the `--set-upstream` flag (or `-u` in short), and <i class="fab fa-git"></i> will display a little more information.
...
remote:      https://github.com/OBDS-Training/OBDS_Training_May_2021/pull/new/usage-test
remote: 
To git@github.com:OBDS-Training/OBDS_Training_May_2021.git
...
- We will use GitHub to manage the `master` branch centrally.
- Every other repository will only ever `pull` the `master` branch from GitHub.
- No one will ever directly `push` their `master` branch to GitHub.
...
For illustrative purposes, to incorporate the changes from branch `kevin` into branch `master` locally:
...
On the page of the repository (<https://github.com/OBDS-Training/OBDS_Training_May_2021>), branches that were recently pushed are highlighted, with a button "Compare & pull request".
...
GitHub also offers a page dedicated to creating pull requests.
...
Often considered cumbersome, in practice manual reviews offer another chance to catch issues and provide feedback, before the changes are applied.
...
**Crucially, a pull request can only be applied if the reviewer selects the "Approve" radio button.**
