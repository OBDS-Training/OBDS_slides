...
`r BiocStyle::Biocpkg("DESeq2")` defines the class `DESeqDataSet` that extends the class `SummarizedExperiment`.

```{r}
dds_se <- as(dds, "SummarizedExperiment")
```
...
::: {style="text-align: center;"}
What slots does the class `DESeqDataSet` add to the base case `SummarizedExperiment`?
:::
