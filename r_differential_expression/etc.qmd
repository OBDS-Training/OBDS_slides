...
The first step of a `r BiocStyle::Biocpkg("DESeq2")` workflow is to construct a `DESeqDataSet` object.
...
`DESeqDataSet` objects can be constructed from various inputs:
...
## Constructing a DESeqDataSet object from a matrix
...
Models the effects of conditions and genotypes independently.
...
Models the effects of conditions and genotypes independently, as well as the effect of their interaction.
...
- Load the `tidyverse` package,
  ...
  Specify `comment = "#"` to skip lines of comments.
...
- Dispersion describes the relationship between a gene's mean expression and its variability across samples.
...
$$y_{i,j} = \beta_{0} + \beta_{dex}X_{dex} + \beta_{cell}X_{cell} + \epsilon_{i,j}$$
:::
