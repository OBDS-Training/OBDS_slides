...
- Consider trying different values for `method=` argument, e.g. 'LR'.
...
- The functions `dply::group_by()` and `dplyr::slice_max()` can be used to extract the genes with the highest log fold-change in each cluster.
