...
- For `FindNeighbors()`, set the arguments `reduction = "integrated.cca"`, `dims = 1:30`, and `reduction.name = "umap.cca"`.
...
#| fig-width: 14
#| fig-height: 7
wrap_plots(c(plots_unintegrated, plots_cca, plots_harmony, plots_mnn), ncol = 4, byrow = FALSE)
```
...
```{r}
reduction_names <- c("pca", "integrated.cca", "integrated.rpca", "harmony", "integrated.mnn")
batch_variable <- "orig.ident"

median_lisi <- vapply(reduction_names, USE.NAMES = TRUE, FUN.VALUE = numeric(1), FUN = function(reduction){
  lisi_mx <- lisi::compute_lisi(Embeddings(seu, "reduction"), meta_data = obj[[]], label_colnames = batch_variable)
  return(median(lisi_mx[,1]))
})
```

```{r}
median_lisi_tidy <- enframe(median_lisi, name = "reduction", value = "median_lisi") %>% 
  arrange(desc(median_lisi))
```

```{r}
ggplot(median_lisi_tidy, aes(reduction, median_lisi)) +
  geom_col()
```
