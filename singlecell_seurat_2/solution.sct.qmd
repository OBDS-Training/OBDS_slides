...
It is given to you again to make sure we all start from the same point.
...
This is good practice in a notebook, to keep a record of how your object looks at different stages of the analysis.
...
- Use `SCTransform()` to apply the SCTransform method to the Seurat object.
...
- The `SCT` assay may have less features because `SCTransform()` calls `sctransform::vst()` with default argument `min_cells = 5`, which only retains features detected in at least 5 cells.
...
- Use `VlnPlot()` to compare the total counts in the RNA and SCT assays, and to compare the number of features detected in each assay.
...
- Repeat the same for the number of features detected in the two assays.
...
- Use `VariableFeatures()` to display the first `10` variable genes in the `SCT` assay.

**Hint:**
Refer to the help page of the function to identify the argument that specified which assay to access.
...
- Combine the code above with `length()` and `intersect()` to display the number of genes that overlap.
...
- First, display the active assay (again!).

It is best practice to check (and set) the active assay before key steps when working with Seurat.
