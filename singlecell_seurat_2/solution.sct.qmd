...
- Use `readRDS()` to import a preprocessed object from the file `seurat_after_qc.rds`.
...
**Note:** The 'SCT' assay may have less features because SCTransform() calls sctransform::vst() with default argument min_cells = 5, which means to use only that have been detected in at least 5 cells.
