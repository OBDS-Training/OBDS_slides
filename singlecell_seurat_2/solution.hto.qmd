...
```{r}
Idents(seurat_citeseq) <- "HTO_classification.global"
VlnPlot(seurat_citeseq, features = "nCount_RNA", pt.size = 0, log = TRUE, split.by = "HTO_maxID")
# VlnPlot(seurat_citeseq, features = "nCount_RNA", pt.size = 0, log = TRUE)
# VlnPlot(seurat_citeseq, features = "nCount_HTO", pt.size = 0, log = TRUE)
```
...
```{r}
seurat_citeseq_singlets <- seurat_citeseq[, seurat_citeseq$HTO_classification.global == "Singlet"]
...
seurat_citeseq_singlets <- RunPCA(seurat_citeseq_singlets)
# Replace '-A', '-B', and '-C' by nothing, i.e. remove those suffixes
...
# Plot
UMAPPlot(seurat_citeseq_singlets)
```
