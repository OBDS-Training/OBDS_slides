...
**Note:**
  How do you read the output?
  Which droplets would you consider doublets?
...
```{r}
table(seu$DF.classifications_0.25_0.01_12)
```

```{r, message=FALSE}
seu <- doubletFinder_v3(seu, PCs = 1:10, pN = 0.25, pK = 0.01, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.01_12", sct = FALSE)
```
...
doublets_table <- data.frame(
  ...
  DoubletFinder = factor(seu$DF.classifications_0.25_0.01_12, c("Singlet", "Doublet"))
)
...
plotDimReduceFeature(as.matrix(logcounts(sce_decontX)),
  dim1 = umap[, 1],
  dim2 = umap[, 2],
...
<!--
It looks like recoding multiple clusters into one -- e.g. Tcells = c(3, 4) -- loses that cluster from the bar plot.
Make a reproducible example for them.
-->
