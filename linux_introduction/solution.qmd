...
5.Change into the resources directory.
...
```
3.Copy **SRR2121685h_1.fastq** from the resources directory to the current directory.
...
6.Copy the directory **/<path>/resources/1_linux/1_linux_intro/** to your course working directory (using `-r` option).

```{bash, eval=FALSE}
cp -r /<path>/resources/1_linux/1_linux_intro .
```

7.Move **SRR2121685h_1.fastq** in your course working directory to the copied directory.  
...
9.Create a new directory within your home directory called **obds**
...
10.Change into the obds directory & make a copy of **manpage.txt** called **copy.txt**
...
ls -l                     # Notice symlink.fastq pointing to original 
                            # SRR2121685h_1 path and differences in colours
nano symlink.fastq
nano hardlink.fastq
```

14.Delete both links at the same time.

15.Compare the output of these two commands.
...
16.Delete the obds directory, try using `rmdir` then `rm`. 
