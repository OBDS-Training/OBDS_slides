...
# Bioconductor single-cell workflow - Import
...
# Bioconductor single-cell workflow - QC & filtering
...
- `scater::addPerCellQC()` and `scater::addPerFeatureQC()` compute the same metrics, but adds them to the `SingleCellExperiment` object as `colData` and `rowData`, respectively, rather than returning them as a new `DataFrame`.

## Visualisation

- `scater::plotScater()` and other functions can help you visualise quality metrics.

- `r BiocStyle::CRANpkg("ggplot2")` gives you the greatest flexibility to visualise information stored in the `colData()` and `rowData()` (you will need to convert from `DataFrame` to `data.frame`).
...
The packages `r BiocStyle::Biocpkg("scater")` and `r BiocStyle::Biocpkg("scran")` provide a range of methods focused on the normalisation and preprocessing of single-cell gene expression data.
