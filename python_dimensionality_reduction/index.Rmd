...
.center[
**Why would we want to do this? Can we standardise _all_ the variables in the data set?**
]
...
We will use the [`StandardScaler`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html) object in the `scikit-learn` package, to standardize features by removing the mean and scaling to unit variance.
...
Use the method [`fit_transform()`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html#sklearn.preprocessing.StandardScaler.fit_transform) method of the `StandardScaler` object to standardise the set of measurements (we called it `x`).
Assign the standardised data to a new variable `x_norm`.

.center[
**Can you see the difference between `x_norm` with `x`?**
]
...
Use `pd.DataFrane` to reconstruct a data frame from `x_norm` and the names of each feature measured.
