...
We will use the [`PCA`](https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.PCA.html) module in the `scikit-learn` library, to perform principal component analysis (PCA).
...
2. Use the `fit_transform()` function of the `pca` object to perform the analysis on the standardised data set.
...
Use the return value of `fit_transform()` to create a `pandas.DataFrame` called `principalDf`.
