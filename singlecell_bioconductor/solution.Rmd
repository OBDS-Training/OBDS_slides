...
- Compute and visualise quality control metrics (library size, genes detected, mitochondrial fraction); use `r BiocStyle::Biocpkg("scater")`.
  ...
  + Identify mitochondrial genes and pass those to the `subsets` argument of the `scater::addPerCellQC()` function.
...
- Visualise quality metrics (three violin plots); use `r BiocStyle::CRANpkg("ggplot2")`.
...
- Convert the counts into normalized expression values to eliminate cell-specific biases (e.g., in capture efficiency); use `scater` and/or `scran`.
...
- When would you rather use `scran::computeSumFactors()` instead?
