...
sce <- DropletUtils::read10xCounts(samples = c(
    "filtered" = "/storage/shared/resources/single_cell/bioconductor/filtered_feature_bc_matrix"
))
```
...
> x-axis is number of cells with non-zero counts for feature
> y-axis is log10 of mean expression across cells
...
```{r}
set.seed(1234)
sce <- scater::runPCA(sce, subset_row=hvg, scale=TRUE)
reducedDimNames(sce)
```
