...
sce <- DropletUtils::read10xCounts(samples = c(
    "filtered" = "data/10x_pbmc5k_nextgem/filtered_feature_bc_matrix"
))
```
...
- Apply PCA; use `fixedPCA()`.
  Set a seed to control reproducibility.
  List the names of dimensionality reduction results available.

**Note:** only give the set of highly variable genes to the `scran::runPCA()` function, to save time, memory, and to focus on biologically informative genes in the data set.
