...
```{r}
keep_rows <- assay(airway, "logcounts") %>% 
  rowVars() %>% 
  order(decreasing = TRUE) %>% 
  head(500)
data_pca <- assay(airway, "logcounts")[keep_rows, ] %>% 
  t() %>% 
  prcomp()
scree_table <- tibble(
  sdev = data_pca$sdev,
  PC = seq_along(data_pca$sdev),
...
#| fig-align: center
#| fig-height: 6
keep_samples <- rownames(data_pca$x)
data_pca$x %>%
  as_tibble() %>% 
