...
- From `pca_iris`,
  extract the coordinates projected on principal components,
  convert them to a *data.frame*,
  and assign the result to an object called `pca_iris_dataframe`.
...
- Store the PCA plot as an object named `pca_iris_plot`.

```{r}
pca_iris_plot <- ggplot(pca_iris_dataframe, aes(PC1, PC2)) +
  geom_point() +
  cowplot::theme_cowplot()
```
...
- Color data points according to their class label.
  Store the plot as an objet called `pca_iris_species`.
