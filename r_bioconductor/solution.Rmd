...
The package `BiocManager` is the only official Bioconductor package on CRAN.
...
- The `Biostrings` package provides the object `DNA_ALPHABET`.
...
- The `Biostrings` package provides the function `IlluminaQuality()`.
...
- Access the package vignette(s).
  How many vignette does the `Biostrings` package include?
...
```{r}
if (!file.exists("data/Homo_sapiens.GRCh38.104.gtf.gz")) {
  download.file(
    url = "http://ftp.ensembl.org/pub/release-104/gtf/homo_sapiens/Homo_sapiens.GRCh38.104.gtf.gz", 
    destfile = "data/Homo_sapiens.GRCh38.104.gtf.gz")
}
```
