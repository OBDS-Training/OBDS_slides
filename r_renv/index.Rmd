...
- In **Directory name:**, type `r_ggplot2`.
...
```{r, include=TRUE, echo=FALSE, fig.align='center', out.height='550px'}
knitr::include_graphics("img/project-blank.png")
```

---

# How new project are set up in RStudio
...
```{r, include=TRUE, echo=FALSE, fig.align='center', out.height='400px', out.width='500px'}
...
**Note:** this installs `r BiocStyle::CRANpkg("renv")` in your _system_ R library, i.e. it is available outside of any project, as soon as you open <i class="fab fa-r-project"></i>.
...
* Lockfile written to '~/github/kevinrue/obds-training-jan2021/r_ggplot2/renv.lock'.

Restarting R session...

* Project '~/github/kevinrue/obds-training-jan2021/r_ggplot2' loaded. [renv 0.12.5]
```
...
- base packages, in the system library (installed with <i class="fab fa-r-project"></i>).

- the `r BiocStyle::CRANpkg("renv")` package, installed in the project library.
...
One of `r BiocStyle::CRANpkg("renv")`’s primary features is the use of a global package cache, which is shared across all projects using `r BiocStyle::CRANpkg("renv")`.

The renv package cache provides two primary benefits:

- Speed - Future calls to `renv::restore()` and `renv::install()` will become much faster, as renv will be able to find and re-use packages already installed in the cache.

- Space - The renv cache should also help you save disk space, because it is not necessary to have duplicate versions of your packages installed in each project.

.pull-left[
**Note:** if you navigate to the `renv/library` subdirectory, you should see that each package is a symbolic link (to the global cache) rather than an actual folder.
]

.pull-right[
```{r, include=TRUE, echo=FALSE, fig.align='center', out.height='300px'}
knitr::include_graphics("img/renv-global-cache.png")
```
]
...
- [GitHub repository](https://github.com/rstudio/renv/)

## git

- [Software Carpentry - Using Git from RStudio](https://swcarpentry.github.io/git-novice/14-supplemental-rstudio/)
