...
_Note:_ The global configuration is stored in the `$HOME/.gitconfig` file.
...
*Key point:* Use `git config` with the `--global` option to configure a user name, email address, editor, and other preferences once per machine.
...
```bash
$ cd /ifs/obds_training/sep20/${USER}
...
_Note:_ Use this repository for your own code and notes throughout the course.
...
```{r, out.height='400px'}
knitr::include_graphics("https://swcarpentry.github.io/git-novice/fig/git-staging-area.svg")
```
...
```{r, , out.height='400px'}
knitr::include_graphics("https://swcarpentry.github.io/git-novice/fig/git-committing.svg")
```
...
```{r, out.height='450px'}
knitr::include_graphics("https://swcarpentry.github.io/git-novice/fig/git-checkout.svg")
```
...
- <i class="fab fa-git"></i> uses the SHA-1 (Secure Hash Algorithm 1) cryptographic algorithm to generate a 40-digit hexadecimal number that uniquely identifies each commit.
  ...
  + _Note:_ <i class="fab fa-git"></i> is smart enough to recognise the identifier from its first 7 characters.
...
_Anecdote:_ "log file" originates from "logbook", itself from "log" (above) + "book", from a wooden float (chip log, or simply log) used to measure speed.
...
```
_Note:_ `git diff` alone shows what you _could_ tell <i class="fab fa-git"></i> to further add to the index but you still haven’t.
Instead, The `--cached` option shows the changes that <i class="fab fa-git"></i> would record if you commit right now.
...
```{r, out.height='250px'}
knitr::include_graphics("https://www.git-tower.com/learn/media/pages/git/ebook/en/command-line/advanced-topics/undoing-things/-416978476-1597909835/revert-concept.png")
```
...
_WARNING:_ This approach is strongly discouraged on shared (i.e. collaborative) projects.
It rewrites the history of the repository.

```{r, out.height='250px'}
knitr::include_graphics("https://www.git-tower.com/learn/media/pages/git/ebook/en/command-line/advanced-topics/undoing-things/413538980-1597909835/reset-concept.png")
```
...
_WARNING:_ This approach rewrites the history of the repository.
...
```bash
$ cd /ifs/obds-training/sep20/${USER}
